...
class AccountMove(models.Model):
    ...
    def _check_case_3(self):
        if self.invoice_line_ids:
            for line in self.invoice_line_ids and self.partner_id.tax_type == "register":
    ...
    def compute_after_tax_wht(self):
        ...
        if self.case1 == True:
            total = ((self.total_tax_amount / 100) * self.after_wht)
    ...
    def action_calculate(self):
        if self.case1 == True:
            ...
            income_partner = self.env['res.partner'].search([('name', '=', 'Income Tax Payable')], limit=1)
            sales_partner = self.env['res.partner'].search([('name', '=', 'Sales Tax Payable')], limit=1)
            ref = self.name
            total_income_tax = 0
            total_sale_tax = 0
    ...
    def _recompute_global_discount_lines(self):
        ''' Compute the dynamic Income Tax lines of the journal entry.'''
        self.ensure_one()
        in_draft_mode = self != self._origin
